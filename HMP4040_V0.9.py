...
+ Calculated Power (VxI) + Save Excel + GL_SpectroSoft - Lab automation (Measure -> fill V/I/P -> OK)

LEAN goals:
- Minimal changes to the already-working ramp logic.
- No extra integration panels. No JSON handshakes. One single software.
- Flow per step:
  1) Apply step
  2) Stabilize (countdown)
  3) Capture PSU snapshot (MEAS V/I) and compute P = V*I
  4) Trigger GL: click Measure, wait popup, fill Voltage->Current->Power, OK, wait popup closed
  5) If GL succeeds: proceed to HOLD and wait for NEXT STEP (manual)  [as requested workflow safety]
     (If you want auto-advance, tell me and I’ll add a single bool.)
  6) NEXT STEP advances without changing PSU during HOLD.

Install:
  python -m pip install pyvisa pyvisa-py openpyxl pywinauto
"""
...
# ---------------------------
# GL SpectroSoft constants (English UI)
# ---------------------------
GL_MAIN_TITLE = "GL_SpectroSoft - Lab"     # keyword match
GL_DIALOG_TITLE = "Luminous efficacy"
GL_MEASURE_BUTTON_TEXT = "Measure"
GL_POPUP_TIMEOUT_S = 12.0
GL_CLOSE_TIMEOUT_S = 10.0
...
class RampGUI(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("HMP4040 Ramp Frontend (CV/CC) + Status + Timer + Snapshots + GL Measure")
    ...
    def _build_ui(self):
        ...
        ttk.Label(frm, text=f"(Fixed limits: {self.V_MAX_PER_CH}V/ch, {self.I_MAX_PER_CH}A/ch)").grid(row=3, column=2, sticky="w")
        ...
        self.preview = tk.Text(frm, width=112, height=13)
    ...
    # ---------- GL SpectroSoft automation (LEAN, English UI) ----------
    ...
    def _gl_click_measure(self, main_win):
        main_win.set_focus()
        try:
            btn = main_win.child_window(title=GL_MEASURE_BUTTON_TEXT, control_type="Button")
    ...
    def _gl_wait_dialog_open(self):
        t0 = time.time()
        while time.time() - t0 < GL_POPUP_TIMEOUT_S:
            time.sleep(0.15)
            dialogs = Desktop(backend="uia").windows(control_type="Window")
            for d in dialogs:
                title = (d.window_text() or "")
                if GL_DIALOG_TITLE.lower() in title.lower():
                    return d
        raise RuntimeError("GL popup 'Luminous efficacy' did not appear.")

    def _gl_wait_dialog_closed(self):
        t0 = time.time()
        while time.time() - t0 < GL_CLOSE_TIMEOUT_S:
            time.sleep(0.15)
            dialogs = Desktop(backend="uia").windows(control_type="Window")
            found = False
            for d in dialogs:
    ...
    def _gl_fill_and_ok(self, dlg, voltage_v, current_a, power_w):
        dlg.set_focus()

        edits = dlg.descendants(control_type="Edit")
        if len(edits) < 3:
            raise RuntimeError(f"Expected 3 Edit controls, found {len(edits)}.")

        # Based on your UI order:
        # edits[0]=Power, edits[1]=Current, edits[2]=Voltage
        edit_power = edits[0]
        edit_current = edits[1]
        edit_voltage = edits[2]

        Vtxt = f"{float(voltage_v):.3f}"
        Itxt = f"{float(current_a):.4f}"
        Ptxt = f"{float(power_w):.4f}"
    ...
    def gl_measure_once(self, voltage_v, current_a, power_w):
        main = self._gl_find_main()
        self._gl_click_measure(main)
        dlg = self._gl_wait_dialog_open()
        self._gl_fill_and_ok(dlg, voltage_v, current_a, power_w)
        if not self._gl_wait_dialog_closed():
            raise RuntimeError("GL popup did not close after OK (not confirming saved measurement).")

    # ---------- Snapshot (measured V/I; P calculated) ----------
    ...
    def _run_worker(self, values, stab, other):
        try:
            ...
            for idx, x in enumerate(values, start=1):
                ...
                # Snapshot after stabilization
                self._ui_set("Capturing snapshot…", "", "Reading V / I from PSU…")
                rec = self._read_snapshot()
                self.records.append(rec)
                self._update_preview()

                # GL Measure immediately after snapshot
                try:
                    self._ui_set("GL Measuring…", "", "Measure -> fill V/I/P -> OK")
                    self.gl_measure_once(
                        voltage_v=rec["meas_v_total"],
                        current_a=rec["meas_i_total"],
                        power_w=rec["meas_p_total"],
                    )
                    self._ui_set("GL Saved", "", "Measurement saved in GL.")
                except Exception as e:
                    self._ui_set("GL Error", "", f"{e}")
                    messagebox.showwarning(
                        "GL automation failed",
                        f"{e}\n\nBring GL window to front (not minimized), then press NEXT STEP to continue manually."
                    )
    ...
    def _finish_run(self):
        ...
        self.btn_stop.config(state="disabled")
        self.btn_next.config(state="disabled" if not self.connected else "normal")
