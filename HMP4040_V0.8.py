...
+ latest_psu.json output + GL handshake auto-next

Key points:
- HMP4040 does NOT support POWER readback via SCPI.
- Snapshots always save measured Voltage/Current from PSU (MEAS:VOLT?, MEAS:CURR?).
- Power is calculated as DC: P_total = V_total * I_total (clearly labeled in Excel).
- After every snapshot, writes latest_psu.json (for downstream tools like GL automation).
- During HOLD, writes gl_handshake.json = {"state":"HOLD", ...}
  and can advance automatically when gl_handshake.json becomes {"state":"DONE","step":<idx>}.
...
def compute_totals(wiring_mode, v1, i1, v2=0.0, i2=0.0):
    """
    Totals for V and I depend on wiring mode.
    """
...
class RampGUI(tk.Tk):
    ...
    def _write_latest_json(self, v_total, i_total, p_total):
        payload = {
            "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
            "step": int(self.current_step_index),
            "steps_total": int(self.current_steps_total),
            "mode": self.mode.get(),
            "wiring": self.wiring_mode,
            "voltage_v": float(v_total),
            "current_a": float(i_total),
    ...
    def _publish_hold_state(self, idx, total):
        try:
    ...
    def _consume_done_and_go_idle(self, idx):
        try:
            GL_HANDSHAKE_PATH.write_text(json.dumps({
                "state": "IDLE",
                "step": int(idx),
    ...
    def _read_snapshot(self):
        ...
        self._write_latest_json(v_total, i_total, p_total)
    ...
    def _run_worker(self, values, stab, other):
        try:
            ...
            for idx, x in enumerate(values, start=1):
                ...
                # Auto snapshot after stabilization
                self._ui_set("Capturing snapshot…", "", "Reading V / I from PSU…")
                rec = self._read_snapshot()
                self.records.append(rec)
                self._update_preview()
                ...
                while True:
                    ...
                    try:
                        if GL_HANDSHAKE_PATH.exists():
                            data = json.loads(GL_HANDSHAKE_PATH.read_text(encoding="utf-8"))
                            if data.get("state") == "DONE" and int(data.get("step", -1)) == int(idx):
                                self._consume_done_and_go_idle(idx)
                                break
                    except Exception:
                        pass
    ...
    def _update_preview(self):
        ...
        for r in tail:
            ...
            lines.append(
                ...
                f"MEAS V={r['meas_v_total']:.4g}  I={r['meas_i_total']:.4g}  P={ptxt:.4g}"
            )
