...
- Manual hold between steps: waits for NEXT STEP after warmup
...
class HMP4040Controller:
    ...
    def select_channel(self, ch: int):
        # HMP series SCPI often supports: INST OUT1..OUT4
    ...
    def apply_cv(self, ch: int, voltage: float, current_set: float):
        # CV: setpoint VOLT, and CURR acts as limit in PSU; UI calls it "Current set".
        self.select_channel(ch)
        self._write(f"VOLT {voltage}")
        self._write(f"CURR {current_set}")

    def apply_cc(self, ch: int, current_set: float, voltage_set: float):
        # CC: setpoint CURR; VOLT is compliance (UI calls it "Voltage set").
        self.select_channel(ch)
        self._write(f"CURR {current_set}")
    ...
    def read_meas_power(self, ch: int) -> float:
        """
        Different firmware sometimes uses slightly different mnemonics.
        We'll try a few. If all fail, raise a clear error.
        """
        self.select_channel(ch)
        candidates = [
            "MEAS:POW?",
        ...
        raise RuntimeError(
            ...
            f"Your HMP4040 may use a different SCPI command. Last error: {last_err}"
        )
...
class RampGUI(tk.Tk):
    ...
    def __init__(self):
        super().__init__()
        self.title("HMP4040 Ramp Frontend (CV/CC) + Manual Hold + Snapshots")
    ...
    def _build_ui(self):
        ...
        # Buttons row 1
        btns1 = ttk.Frame(frm)
        btns1.grid(row=15, column=0, columnspan=4, sticky="ew", pady=10)
        for c in range(6):
            btns1.columnconfigure(c, weight=1)

        self.btn_connect = ttk.Button(btns1, text="CONNECT", command=self.connect)
        self.btn_start = ttk.Button(btns1, text="START", command=self.start, state="disabled")
        self.btn_next = ttk.Button(btns1, text="NEXT STEP", command=self.next_step, state="disabled")
        self.btn_snapshot = ttk.Button(btns1, text="SNAPSHOT NOW", command=self.snapshot_now, state="disabled")
        self.btn_stop = ttk.Button(btns1, text="STOP (soft)", command=self.stop_soft, state="disabled")
        self.btn_estop = ttk.Button(btns1, text="E-STOP (OFF now)", command=self.estop, state="disabled")
        self.btn_disconnect = ttk.Button(btns1, text="DISCONNECT", command=self.disconnect, state="disabled")
        ...
        self.btn_disconnect.grid(row=1, column=0, padx=5, sticky="ew")
        ttk.Button(btns1, text="SAVE EXCEL", command=self.save_excel).grid(row=1, column=1, padx=5, sticky="ew")
        ttk.Button(btns1, text="CLEAR DATA", command=self.clear_data).grid(row=1, column=2, padx=5, sticky="ew")

        # A tiny table preview (last 5 rows)
        ...
        self.preview = tk.Text(frm, width=92, height=10)
    ...
    def _apply_step_setpoints(self, x, other):
        ...
        if self.wiring_mode == "SERIES":
            # Robust split:
            # Total V = V1+V2, keep CH1 <= 32 V, remainder on CH2
            if mode == "CV":
                ...
                # CC: current same through series; split VOLT compliance robustly
                i_set = x
        ...
        if self.wiring_mode == "PARALLEL":
            # Split current between channels, same voltage on both
            if mode == "CV":
                v = x
                i1 = other / 2.0
                i2 = other / 2.0
    ...
    def _read_snapshot(self):
        """
        Reads V/A/P per channel (measured) and computes totals based on wiring.
    ...
    def snapshot_now(self):
        ...
        try:
            ...
            self.status.set("Snapshot captured.")
        except Exception as e:
    ...
    def start(self):
        ...
        try:
            ...
            stab = self._parse_float(self.var_stab.get(), "warmup/stabilization time")
        ...
        t = threading.Thread(target=self._run_ramp_worker, args=(values, stab, other), daemon=True)
        t.start()
    ...
    def stop_soft(self):
        if self.running:
            ...
            # Also release wait if stuck in hold
            self.next_event.set()
    ...
    def _run_ramp_worker(self, values, warmup_s, other):
        try:
            ...
            for idx, x in enumerate(values, start=1):
                ...
                # Manual HOLD for GL Spect actions
                self.waiting_for_next = True
                self.next_event.clear()
                self.step_status.set(
                    f"HOLD at Step {idx}/{len(values)} — do GL Spect clicks, then press NEXT STEP."
                )
                ...
                # Capture snapshot at the moment you press NEXT
                try:
                    rec = self._read_snapshot()
                    self.records.append(rec)
                    self._update_preview()
                except Exception as e:
                    # Don’t crash the run; just warn and continue
                    messagebox.showerror("Snapshot error", str(e))
