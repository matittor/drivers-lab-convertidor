...
- Keep PSU control logic intact.
- GL automation: ONLY press SPACE (your confirmed trigger), wait N seconds, then proceed to next step.
"""
...
class RampGUI(tk.Tk):
    ...
    def gl_trigger_space_and_wait(self, wait_s: float):
        # Keep exactly the trigger you already have (SPACE), then wait and proceed.
        ...
        send_keys("{SPACE}")  # <-- trigger measurement
        time.sleep(0.2)
    ...
    def _run_worker(self, values, stab, other):
        try:
            ...
            for idx, x in enumerate(values, start=1):
                ...
                # GL trigger: SPACE, wait, then auto-advance
                try:
