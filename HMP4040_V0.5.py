"""
HMP4040 Ramp Frontend (CV/CC) + Manual Hold + Auto Snapshot After Stabilization
- Clear status + countdown timer
- TEST POWER SCPI button to find correct command on your PSU
- Power is READ directly from PSU (no calculated power)
...
class HMP4040Controller:
    def __init__(self):
        ...
        self.inst = None
        self.idn = ""
        self.power_cmd = None  # detected SCPI command for power
    ...
    def _query(self, cmd: str, timeout_ms: int | None = None) -> str:
        if not self.inst:
            raise RuntimeError("Instrument not connected.")
        old = None
    ...
    def _query_float(self, cmd: str, timeout_ms: int | None = None) -> float:
        return float(self._query(cmd, timeout_ms=timeout_ms).strip())
    ...
    def detect_power_command(self) -> str | None:
        """
        Try a set of likely SCPI commands for measured power.
        Returns the working command, or None if none works.
        Uses short timeout so it doesn't freeze the UI.
        """
        if not self.inst:
            raise RuntimeError("Instrument not connected.")

        candidates = [
            # Common patterns across supplies/firmwares:
            "MEAS:POW?",
            "MEAS:POWer?",
            "FETC:POW?",
            "FETC:POWer?",
        ...
        for cmd in candidates:
            try:
                # select ch1 and try quickly
                self.select_channel(1)
                _ = self._query_float(cmd, timeout_ms=800)
                self.power_cmd = cmd
                return cmd
            except Exception as e:
                last_err = e
        ...
        # Optional: fetch instrument error to help you diagnose
        try:
            err = self._query("SYST:ERR?", timeout_ms=800).strip()
        except Exception:
            err = None

        if err:
            raise RuntimeError(f"No POWER SCPI command worked. Last error: {last_err}. SYST:ERR? = {err}")
        else:
            return None

    def read_meas_power(self, ch: int) -> float:
        if not self.power_cmd:
            raise RuntimeError("Power command not detected. Click 'TEST POWER SCPI' first.")
        self.select_channel(ch)
        return self._query_float(self.power_cmd)
...
class RampGUI(tk.Tk):
    ...
    def __init__(self):
        ...
        self.power_info = tk.StringVar(value="Power SCPI: (not tested)")
    ...
    def _build_ui(self):
        ...
        ttk.Label(status_box, textvariable=self.power_info).grid(row=3, column=0, columnspan=2, sticky="w")
        ...
        ttk.Label(frm, text=f"(Fixed limits: {self.V_MAX_PER_CH}V/ch, {self.I_MAX_PER_CH}A/ch)").grid(row=3, column=2, columnspan=2, sticky="w")
        ...
        self.btn_test_power = ttk.Button(btns, text="TEST POWER SCPI", command=self.test_power_scpi, state="disabled")
        ...
        self.btn_test_power.grid(row=0, column=1, padx=4, sticky="ew")
        self.btn_start.grid(row=0, column=2, padx=4, sticky="ew")
        self.btn_next.grid(row=0, column=3, padx=4, sticky="ew")
        self.btn_snapshot.grid(row=0, column=4, padx=4, sticky="ew")
        self.btn_stop.grid(row=0, column=5, padx=4, sticky="ew")
        self.btn_estop.grid(row=0, column=6, padx=4, sticky="ew")
        self.btn_disconnect.grid(row=1, column=0, padx=4, sticky="ew")
        ...
        self.preview = tk.Text(frm, width=98, height=12)
    ...
    def connect(self):
        ...
        try:
            ...
            self.power_info.set("Power SCPI: (not tested)  → click TEST POWER SCPI")
            self._ui_set("Connected", "", "Ready. Set parameters, then START.")
            self.btn_test_power.config(state="normal")
    ...
    def test_power_scpi(self):
        if not self.connected:
            messagebox.showwarning("Not connected", "Connect first.")
            return
        self._ui_set("Testing POWER…", "", "Trying SCPI commands (fast)…")
        try:
            cmd = self.ctrl.detect_power_command()
            if cmd:
                self.power_info.set(f"Power SCPI: {cmd}")
                self._ui_set("Connected", "", "POWER command detected. Ready.")
            else:
                self.power_info.set("Power SCPI: (none found)")
                self._ui_set("Connected", "", "No POWER command found on this PSU/firmware.")
                messagebox.showerror(
                    "POWER not supported",
                    "No SCPI power readback command worked on this unit.\n"
                    "If you want, we can add a safe fallback: P = V_meas * I_meas."
                )
        except Exception as e:
            self._ui_set("Connected", "", "Power test error.")
            messagebox.showerror("Power SCPI test error", str(e))
    ...
    def disconnect(self):
        ...
        self.connected = False
        self.idn.set("")
        self.power_info.set("Power SCPI: (not tested)")
        self._ui_set("Ready", "", "Not connected.")
        self.btn_test_power.config(state="disabled")
    ...
    def snapshot_now(self):
        if not self.connected:
            ...
            return
        try:
            self._ui_set("Capturing snapshot…", "", "Reading V / I / P from PSU…")
    ...
    def start(self):
        if not self.connected:
            ...
            return
        if not self.ctrl.power_cmd:
            messagebox.showwarning("POWER not ready", "Click TEST POWER SCPI first (needed for snapshots).")
            return
        if self.running:
